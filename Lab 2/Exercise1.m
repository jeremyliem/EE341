...
%create horizontal sobel edgedectection
h2 = [1 2 1;0 0 0;-1 -2 -1];

%Convolution of M1 with DailyShow
M1 = conv2(h1,image2);
abs_M1 = abs(uint8(M1));

%convolution of M2 with DailyShow
M2 = conv2(h2,image2);
abs_M2 = abs(uint8(M2));
...
%This is the way using imfilter.
%{
img = imread('DailyShow.jpg', 'jpeg');
% 8-bit gray-scale
img2 = rgb2gray(img);
size(img2);
%dimension dailyshow = 400 x 468
%Skernel vertical edge 
h1 = [-1 0 1; -2 0 2; -1 0 1];
%horizontal
h2 = [1 2 1; 0 0 0; -1 -2 -1];
%convolve
plot(2)
M1=imfilter(img2,h1,'same');
subplot(2,2,1);
imshow(M1,[])

M2=imfilter(img2,h2,'same');
subplot(2,2,2);
imshow(M2,[])

M3=sqrt(double(M1).^2+double(M2).^2);
subplot(2,2,3);
imshow(M3,[])
%}
